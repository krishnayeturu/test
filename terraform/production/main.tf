terraform {
  backend "s3" {
    ...
    key     = "platform-automation/ms-wrkr-admissions/production/terraform.tfstate"
    region  = "us-west-2"
    encrypt = true
...
provider "aws" {
  ...
  allowed_account_ids = ["539365964353"] # placeholder til we have Platform Automation production account access
}
# endregion providers
...
locals {
  ...
  tags_as_map = {
    Owner            = "DataPlatform"
    Stage            = "production"
    Repository       = "https://gitlab.com/2ndwatch/microservices/workers/ms-wrkr-admissions/tree/master/terraform/production"
...
resource "aws_dynamodb_table" "permissions_table" {
  ...
  /* read_capacity  = 5 # initial estimate for test case, changeable as we determine necessary
  write_capacity = 3 # initial estimate for test case, changeable as we determine necessary */
  hash_key  = "ID"
